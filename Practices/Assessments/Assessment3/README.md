...
def chris_is_missed
  MovieDatabase.execute(<<-SQL)
    SELECT
      CF_movie.title, lead_actor.name
    FROM
      movies CF_movie
    JOIN castings CF_casting
      ON CF_casting.movie_id = CF_movie.id
    JOIN actors CF_actor
      ON CF_actor.id = CF_casting.actor_id
    JOIN castings lead_casting
      ON lead_casting.movie_id = CF_casting.movie_id
    JOIN actors lead_actor
      ON lead_actor.id = lead_casting.actor_id
    WHERE
      CF_actor.name = 'Chris Farley' AND
      lead_casting.ord = 1
